    ...
    if (_online) {
        $.ajax({
            ...
            error: function (event, request, settings) {
                if (repeat_on_error) setTimeout(function () {
                    console.log('Trying again request for: ', params, " Error:", status);
                    api(params, callback, cache, on_encrypt, repeat_on_error);
                }, 15 * 1000);
...
var init_plugins = function ($) {
    ...
    $.fn.search_autocomplete = function (callback, callback2) {
        ...
        var $that = $(this);
        api('search_auto_complete2', function (res) {
                    ...
                    , {
                        ...
                        templates: {
                            ...
                            suggestion: function(data) {
                                return '<p><strong>' + data.value + '</strong> – ' + moment(data.date).format('DD/MM/YY') + '</p>';
                            }
                        },
                        display: 'value',
                    },
