...
App.View('ToolsOdeco', {
    ...
    breadcrumb: blocks.observable([{title: 'Inicio', url: '#home'}, {title: 'Herramientas', url: '#home'}, {title: 'Calculadora Odeco', url: ''}]),
});
...
App.View('ToolsManual', {
    ...
    set_type_registers: function(){
        ...
        that.dom.find("#manual_select_type_registers").selectivity({
            items: that.type_registers().map(function(c){c.text = c.title; return c })
        }).on('change', function(){
    ...
    set_areas: function(){
        ...
        $dom.selectivity({
            ...
            }).map(function(c){c.text = c.title; return c })
        }).on('change', function(){
    ...
    breadcrumb: blocks.observable([{title: 'Inicio', url: '#home'}, {title: 'Herramientas', url: '#home'}, {title: 'Manual de Reclamos', url: ''}]),
...
App.View('ToolsLdi', {
            ...
            if (!this.inited()) {
                ...
                api(get_params('tools', {type: 'ldi_init'}), function (res) {
                    ...
                    that.dom.find("#ldi_select_country").selectivity({
                        items: res.countries.map(function(c){ return c.country }),
    ...
    event_filter: function(){
        var that = this;
        preload3();
        api(get_params('tools',{type: 'ldi_search', country: this.value_country(), city: this.value_city() == 'all' ? '' : this.value_city() }), function( res ) {
            that.html_table(res.html);
            preload3(1);
        }, false);
    },
    event_filter_phone: function(){
        var that = this;
        preload3();
        api(get_params('tools',{type: 'ldi_range', phone: this.value_phone(), inter: this.dom.find('#filter_tools_ldi_range .my_radio input:checked').val() == '0' ? 1 : null}), function( res ) {
            that.html_table(res.html);
            preload3(1);
        }, false);
    },
    reset_cities_select: function(){
        ...
        that.dom.find("#ldi_select_city").selectivity({
            items: that.cities().filter(function(c){ return !country || c.value == 'all' || c.country == country}).map(function(c){ return {id: c.value, text: c.city}}),
    ...
    breadcrumb: blocks.observable([{title: 'Inicio', url: '#home'}, {title: 'Herramientas', url: '#home'}, {title: 'LDI Y LDN', url: ''}]),
...
App.View('ToolsSimulador', {
    ...
    routed: function (params) {
        ...
        that.breadcrumb([{title: 'Inicio', url: '#home'}, {title: 'Herramientas', url: '#home'}, {title: 'Simulador Endeudamiento', url: ''}])
